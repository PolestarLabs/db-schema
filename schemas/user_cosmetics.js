...
module.exports = function USER_COSMETICS(activeConnection) {
   ...
   */
  UserCosmeticsSchema.methods.addItem = function (itemId, amt = 1, crafted = false) {
   ...
   * @param {boolean} [crafted=false]
   */
  UserCosmeticsSchema.methods.removeItem = function (itemId, amt = 1, crafted = false) {
   ...
   */
  UserCosmeticsSchema.methods.modifyItems = async function (items, debug) {
    const arrayFilters = [];
    const increments = {};
   ...
   * @returns {boolean}
   */
  UserCosmeticsSchema.methods.hasItem = function (itemId, count = 1) {
   ...
   * @returns {number}
   */
  UserCosmeticsSchema.methods.amtItem = function (itemId) {
    return this.inventory.find((itm) => itm.id === itemId)?.count || 0;
  };

  // ── Model & statics ────────────────────────────────────────────

  const MODEL = activeConnection.model("UserCosmetics", UserCosmeticsSchema, "user_cosmetics");
