/**
 * @deprecated LEGACY USER SCHEMA — monolithic "userdb" collection.
 *
 ...
 *   - schemas/user_analytics.js      (statistics, legacy metrics)
 *   - schemas/user_connections.js    (third-party connections)
 *
 * When sunsetting: delete this file and remove the `_legacyUserDB`
...
/** @deprecated Use schemas/users_core.js and split collections. Scheduled for removal. */
module.exports = function LEGACY_USER_DB(activeConnection){
  ...
  UserSchema.methods.upCommend = function upCommend(USER, amt = 1) {
    const miscDB = require("./_misc/_misc.js")(activeConnection);
  ...
  const MODEL = activeConnection.model("UserDB", UserSchema, "userdb");
  ...
  MODEL.get = function (query, project, avoidNew) {
    return new Promise(async (resolve) => {
      ...
      if (data === null && (query.id || typeof query === 'string') )  return resolve( this.new( await PLX.resolveUser?.(query.id||query) ) );
